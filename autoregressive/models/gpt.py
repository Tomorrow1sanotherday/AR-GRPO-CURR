...
class ModelArgs:
    ...
    token_dropout_p: float = 0.0 #0.1
    attn_dropout_p: float = 0.0
    resid_dropout_p: float = 0.0 #0.1
    ffn_dropout_p: float = 0.0 # 0.1
    drop_path_rate: float = 0.0

    num_classes: int = 1000
    caption_dim: int = 2048
    class_dropout_prob: float = 0.0 # 0.1
    model_type: str = 'c2i'
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        #print(xq.shape,keys.shape,values.shape,mask.shape)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        if idx is not None and cond_idx is not None: # training or naive inference
            ...
            #print("cond_embedd",cond_embeddings)
            ...
            if cond_idx is not None: # prefill in inference
                if cond_idx.ndim==2:
                    cond_idx=cond_idx.squeeze(1)
                #tmp = self.cls_embedding(cond_idx, train=self.training)
                token_embeddings = self.cls_embedding(cond_idx, train=self.training)[:,:self.cls_token_num]
                #print("token_embedd",token_embeddings)
                #print(tmp.shape,self.cls_token_num)
        ...
        #if input_pos is not None and input_pos.max()<3:
